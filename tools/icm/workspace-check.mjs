...
async function approvalFailures(repositoryRoot) {
  ...
  for (const entry of entries) {
    ...
    if (project.approval_contract !== 'artifact-receipts') {
      failures.push(`${projectPath} must declare approval_contract: artifact-receipts`);
      continue;
    }
...
async function configurationFailures(repositoryRoot, config, configPresent) {
  ...
  const environments = config.release?.allowedEnvironments;
  if (
    !Array.isArray(environments)
    || environments.some((environment) => typeof environment !== 'string' || environment.trim() === '')
    || new Set(environments).size !== environments.length
  ) failures.push('icm.config.json release.allowedEnvironments must be a unique list of names');
  return failures;
}
