...
5. Establish whether accepted work or repository policy requires full integration proof. If not, record a scoped N/A. If required, resolve premature corrections, then run the repository's full proof once on the exact candidate; a team setup uses the candidate gate its kit adds to this stage. Disabled configuration or PR prose cannot waive required proof.
...
| [`testing-rules.md`](../../_shared/engineering/testing-rules.md) | `Earn each test`; `Prove the right cause at a stable boundary`; `Use the efficient proof sequence`; `Focused repository commands` |
